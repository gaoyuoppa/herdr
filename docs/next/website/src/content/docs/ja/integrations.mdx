---
title: インテグレーション
description: Pi、OMP、Claude Code、Codex、Qwen Code、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Cursor Agent CLI、MastraCode、Grok CLI 向けの Herdr インテグレーションをインストールします。
---
...
| --- | --- | --- |
| ライフサイクル権威 | Pi、OMP、Qwen Code、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、MastraCode | インストールされ、そのペインについて能動的に報告している間は、フックまたはプラグインのイベントが `idle`、`working`、`blocked` を決定します。同じライフサイクル権威に対して、Herdr はスクリーンマニフェストのフォールバックを併用しません。 |
...
このフックは、Grok CLI が Herdr ペイン内で動作している間、Grok の `SessionStart` フックを通じてセッション識別を報告します。Grok の状態は Herdr のスクリーンマニフェスト検出から得られます。

Herdr はデフォルトで `~/.grok` を使い、`GROK_HOME` が設定されていればそちらを使います。Grok の設定ディレクトリはあらかじめ存在している必要があります。Grok はそのディレクトリ内のすべての `hooks/*.json` をマージするため、インストールは自己完結した `hooks/herdr.json` と、その隣の `hooks/herdr-agent-state.sh` を書き込み、ほかのフックファイルは編集しません。アンインストールはこの 2 つの Herdr 所有ファイルだけを削除します。
