...
Claude Code、Codex、GitHub Copilot CLI、Droid、Qoder CLI、Cursor Agent CLI 和 Grok CLI 的集成有意不作为生命周期权威。它们为恢复提供原生会话身份,但它们的钩子并不覆盖整个生命周期,可能漏掉权限审批结果、Esc 中断或其他状态转换。对这些智能体,Herdr 仍然使用屏幕清单检测。
