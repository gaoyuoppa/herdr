...
`copy_on_select = true` 和 `copy_on_select = "clipboard"` 保持默认行为：鼠标拖选终端文本后立即复制。设置为 `false` 或 `"manual"` 时，选区会保持高亮，按 `Enter` 或 `y` 复制，按 `Esc` 取消。设置为 `"disabled"` 会禁用鼠标文本选择和复制。

`host_cursor` 控制 Herdr 使用外部终端的原生光标还是自行绘制光标。默认值 `auto` 会在 Windows 和 WSL 上使用绘制光标以减少重绘闪烁，在其他平台使用原生光标。

在 `[ui]` 下设置 `tab_bar_position = "bottom"`，可将桌面标签栏放到终端窗格下方。Prefix、Navigate、Copy 和 Resize 模式栏激活时会暂时替换底部标签栏。默认值为 `"top"`。

如需让 Ctrl+右键手势传给启用鼠标上报的窗格应用，而不是打开 Herdr 菜单，可设置 `right_click_passthrough_modifier = "ctrl"`。
