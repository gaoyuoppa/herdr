...
`copy_on_select = true` 和 `copy_on_select = "clipboard"` 保持默认行为：鼠标拖选或双击终端文本后立即复制。设置为 `false` 或 `"manual"` 时，选区会保持高亮，按 `Ctrl+C`、宿主转发的 `Cmd+C`、`Enter` 或 `y` 复制，按 `Esc` 取消。设置为 `"disabled"` 会完全禁用鼠标文本选择和复制。
