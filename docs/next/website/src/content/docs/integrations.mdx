---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, Qwen Code, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Cursor Agent CLI, MastraCode, and Grok CLI.
---
...
| --- | --- | --- |
| Lifecycle authority | Pi, OMP, Qwen Code, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, MastraCode | When installed and actively reporting for the pane, hook or plugin events author `idle`, `working`, and `blocked`. Herdr does not also use screen manifest fallback for that same lifecycle authority. |
...
The hook records Qwen's session id as live pane identity. Herdr does not currently use it for native session restore because no stable Qwen resume command is wired into Herdr.
