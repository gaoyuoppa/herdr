...
`copy_on_select = true` and `copy_on_select = "clipboard"` keep the default behavior: drag-selecting or double-clicking terminal text copies it immediately. Set it to `false` or `"manual"` to retain the selection until `Ctrl+C`, a host-forwarded `Cmd+C`, `Enter`, or `y` copies it; `Esc` cancels it. Set it to `"disabled"` to disable mouse text selection and copying.
