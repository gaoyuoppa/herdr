...
- **所有智能体状态一目了然**——阻塞、工作中、已完成。看到的是真实终端，而不是经过包装的转述。
- **分离后智能体继续运行**——可以从任意终端或通过 SSH 重新连接，会话在重启后依然保留。
- **智能体也能使用 Herdr**——通过纯 Socket API 创建窗格、读取输出并互相等待。[智能体技能 →](https://herdr.dev/zh-cn/docs/agent-skill/)
