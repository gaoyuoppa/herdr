...
impl App {
    ...
    fn prepare_terminal_key_forward(
        ...
        if self.handle_manual_selection_key(source_id, &key) {
            return None;
        }
    ...
    fn handle_manual_selection_key(&mut self, source_id: InputSourceId, key: &TerminalKey) -> bool {
...
mod tests {
    ...
    #[tokio::test]
    async fn manual_copy_key_suppresses_grouped_and_later_repeats() {
        let (mut app, info) = app_with_screen_bytes(b"alpha beta");
        app.state.copy_on_select = crate::config::CopyOnSelectModeConfig::Manual;
        ...
        let source_id = 41;
        let copy_key = TerminalKey::new(KeyCode::Char('y'), KeyModifiers::empty())
            .with_windows_record(crate::input::WindowsKeyRecord {
                key_down: true,
                repeat_count: 3,
                virtual_key_code: 0x59,
                virtual_scan_code: 0x15,
                unicode: 'y' as u16,
                control_key_state: 0,
            });
        app.route_client_events_from(
            source_id,
            vec![crate::raw_input::RawInputEvent::Key(copy_key.clone())],
            false,
        );
        ...
        app.route_client_events_from(
            ...
            vec![crate::raw_input::RawInputEvent::Key(
                copy_key.clone().with_kind(KeyEventKind::Repeat),
            )],
            false,
        );
        assert!(app.event_rx.try_recv().is_err());
        ...
        app.route_client_events_from(
            source_id,
            vec![crate::raw_input::RawInputEvent::Key(
                copy_key.with_kind(KeyEventKind::Release),
            )],
            false,
        );
