...
pub(super) fn keybind_help_groups(app: &AppState) -> Vec<HelpGroup> {
    ...
    groups.push((
        ...
        vec![
            ...
            help_entry(keybind_label(&kb.settings), t!("state.settings").to_string()),
            help_entry(keybind_label(&kb.detach), t!("state.detach").to_string()),
            help_entry(keybind_label(&kb.reload_config), t!("keybind.reload_config").to_string()),
            help_entry(
    ...
    let workspace_tab = vec![
        help_entry(keybind_label(&kb.workspace_picker), t!("keybind.workspace_navigation").to_string()),
        help_entry(keybind_label(&kb.goto), t!("keybind.session_navigator").to_string()),
        help_entry(keybind_label(&kb.new_workspace), t!("keybind.new_workspace").to_string()),
        help_entry(keybind_label(&kb.new_worktree), t!("keybind.new_worktree").to_string()),
        help_entry(keybind_label(&kb.open_worktree), t!("keybind.open_worktree").to_string()),
        help_entry(
            keybind_label(&kb.remove_worktree),
            t!("keybind.delete_worktree_checkout").to_string(),
        ),
        help_entry(keybind_label(&kb.rename_workspace), t!("keybind.rename_workspace").to_string()),
        help_entry(keybind_label(&kb.close_workspace), t!("keybind.close_workspace").to_string()),
        help_entry(keybind_label(&kb.previous_workspace), t!("keybind.previous_workspace").to_string()),
        help_entry(keybind_label(&kb.next_workspace), t!("keybind.next_workspace").to_string()),
        help_entry(indexed_label(&kb.switch_workspace), t!("keybind.switch_workspace_n").to_string()),
        help_entry(keybind_label(&kb.previous_agent), t!("keybind.previous_agent").to_string()),
        help_entry(keybind_label(&kb.next_agent), t!("keybind.next_agent").to_string()),
        help_entry(indexed_label(&kb.focus_agent), t!("keybind.focus_agent_n").to_string()),
        help_entry(keybind_label(&kb.new_tab), t!("keybind.new_tab").to_string()),
        help_entry(keybind_label(&kb.rename_tab), t!("keybind.rename_tab").to_string()),
        help_entry(keybind_label(&kb.previous_tab), t!("keybind.previous_tab").to_string()),
        help_entry(keybind_label(&kb.next_tab), t!("keybind.next_tab").to_string()),
        help_entry(indexed_label(&kb.switch_tab), t!("keybind.switch_tab_n").to_string()),
        help_entry(keybind_label(&kb.close_tab), t!("keybind.close_tab").to_string()),
    ];
    groups.push((t!("keybind.group_workspaces").to_string(), workspace_tab));

    let panes = vec![
        help_entry(keybind_label(&kb.split_vertical), t!("keybind.split_vertical").to_string()),
        help_entry(keybind_label(&kb.split_horizontal), t!("keybind.split_horizontal").to_string()),
        help_entry(keybind_label(&kb.close_pane), t!("keybind.close_pane").to_string()),
        help_entry(keybind_label(&kb.rename_pane), t!("keybind.rename_pane").to_string()),
        help_entry(keybind_label(&kb.edit_scrollback), t!("keybind.edit_scrollback").to_string()),
        help_entry(keybind_label(&kb.copy_mode), t!("keybind.copy_mode").to_string()),
        help_entry(keybind_label(&kb.zoom), t!("keybind.zoom_pane").to_string()),
        help_entry(keybind_label(&kb.resize_mode), t!("keybind.resize_mode").to_string()),
        help_entry(keybind_label(&kb.toggle_sidebar), t!("keybind.toggle_sidebar").to_string()),
        help_entry(keybind_label(&kb.focus_pane_left), t!("keybind.focus_pane_left").to_string()),
        help_entry(keybind_label(&kb.focus_pane_down), t!("keybind.focus_pane_down").to_string()),
        help_entry(keybind_label(&kb.focus_pane_up), t!("keybind.focus_pane_up").to_string()),
        help_entry(keybind_label(&kb.focus_pane_right), t!("keybind.focus_pane_right").to_string()),
        help_entry(keybind_label(&kb.cycle_pane_next), t!("keybind.cycle_pane_next").to_string()),
        help_entry(
            keybind_label(&kb.cycle_pane_previous),
            t!("keybind.cycle_pane_previous").to_string(),
        ),
        help_entry(keybind_label(&kb.last_pane), t!("keybind.last_pane").to_string()),
    ];
...
pub(super) fn render_keybind_help_overlay(app: &AppState, frame: &mut Frame) {
    ...
    frame.render_widget(
        Paragraph::new(format!(" {}", t!("keybind.subtitle")))
    ...
    frame.render_widget(
        Paragraph::new(Line::from(vec![
            Span::styled(format!(" {} ", t!("common.scroll")), Style::default().fg(app.palette.overlay0)),
            Span::styled("wheel ↑↓", Style::default().fg(app.palette.text)),
            Span::styled("  ·  ", Style::default().fg(app.palette.overlay0)),
            Span::styled(t!("common.jump").to_string(), Style::default().fg(app.palette.overlay0)),
            Span::styled(" pgup / pgdn ", Style::default().fg(app.palette.text)),
            Span::styled("  ·  ", Style::default().fg(app.palette.overlay0)),
            Span::styled(t!("common.close").to_string(), Style::default().fg(app.palette.overlay0)),
