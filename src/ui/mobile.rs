...
fn agent_summary_segments(counts: GlobalAgentCounts) -> Vec<(String, SummaryTone)> {
    ...
    if counts.blocked > 0 {
        segments.push((
            t!("mobile.blocked_count", count = counts.blocked).to_string(),
            SummaryTone::Blocked,
        ));
    }
    if counts.done > 0 {
        segments.push((t!("mobile.done_count", count = counts.done).to_string(), SummaryTone::Done));
    }
    if counts.working > 0 {
        segments.push((t!("mobile.working_count", count = counts.working).to_string(), SummaryTone::Working));
    }
    if counts.idle > 0 {
        segments.push((t!("mobile.idle_count", count = counts.idle).to_string(), SummaryTone::Idle));
    }
    segments
}
